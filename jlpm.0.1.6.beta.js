;(function(win) {
  var doc = win.document,
	...
	jlpm.fn = jlpm.prototype = {
		...
		domFind:function(selector, content){
			...
			if (!this.isString(selector)){
				return this.record(undefined, selector, content, (selector.nodeType ? 1 : selector.length ? selector.length : 0), undefined);
			}else{
				if ( !! this.Doms[selector] && !content) {
					...
					elem = this.find ? content != undefined ? this.find(selector, content || doc) : this.find(selector) : undefined;
				}
				return this.record(undefined, elem, content, (elem.nodeType ? 1 : elem.length ? elem.length : 0), undefined);
			}
		},
		record: function(old, ev, content, length, selector) {
			this.oldElement = old != undefined ? old : this[0] != undefined ? this[0] : doc;
			this.content = content != undefined ? content : ev.parentNode ? ev.parentNode : doc;
			this.length = length != undefined ? length : ev.length ? ev.length : 0;
		...
		hasAttr: function(name, m) { //是否包含此属性
			var m = m ? m : this[0];
			return m ? hasOwn.call(m, name) ? true : (this.attr&&this.attr(name, undefined, m)||m[name]) != undefined ? true : false : false;
		},
		trim: _trim && !_trim.call("\uFEFF\xA0") ?
		function(text) {
	...
	//DOM加载
	jlpm.fn.ready = function(callback) {
		var self = this;
		function ready(){
			if (doc.addEventListener) {
	...
	jlpm.fn.Doms._count = 0;
	jlpm.fn.sMatch.core_pnum = /[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source;
	jlpm.fn.sMatch.rnumnonpx = (function(v) {
		return new RegExp("^(" + v + ")(?!px)[a-z%]+$", "i");
	})(jlpm.fn.sMatch.core_pnum);
	jlpm.fn.sMatch.rnumsplit = (function(v) {
		...
		target.each(obj, function(name, value) {
			if (value){
				target.extend(self.isFunction(obj[name]) ? obj[name] : function() {
					return this.loadon(value&&value["url"] ? value["url"] : "", function() {
						(value&&value["callback"] ? value["callback"] : this.emptyFn)();
	...
	*/
	...
	jlpm.fn.extend({
		...
		ajaxLocParts : jlpm.fn.sMatch.rurl.exec(ajaxLocation.toLowerCase()) || [],
		...
		ajax.fn = ajax.prototype = {
			...
			_readyStateChange: function(type, callback) {
				var self = this;
			...
			send: function(url, method, data, callback) {
				...
				if (result.status && !jlpm.isCdomain(url) && !jlpm.isEmpty(url)) {
					...
					self.xhr.onreadystatechange = function() {
						self._readyStateChange(dataTypes[0], callback);
					};
					if (self.async) {
						self._sto(function() {
							self.cancel("timeout");
			...
			get: function(url, data, callback, options) {
				jlpm.extend(this.options, this);
				jlpm.extend(this.options, this.config);
				return this.send(url, "get", data, callback);
			},
			post: function(url, data, callback, options) {
				jlpm.extend(this.options, this);
				jlpm.extend(this.options, this.config);
				return this.send(url, "post", data, callback);
			}
		};
		ajax.fn.init.prototype = ajax.fn;
		return ajax(options);
	}, "Ajax").each(["get","post"],function(i,name){
		jlpm.fn.extend(function(url, data, callback, options){
			return this.Ajax(options).send(url, "get", data, callback);
		},name);
	}).extend(function(url, callback, options) {
		options = options || {};
					...
					if (this.isString(t) || this.isArray(t)) {
						...
						this.each(va, function(i) {
							...
							self.each(m, function() {
								...
								vt = (vt == "DOMMouseScroll" || vt == "mousewheel") ? (ua().browser == "mozilla") ? "DOMMouseScroll" : "mousewheel" : (ua().type == "desktop") ? (vt == "mousedown") ? "touchstart" : (vt == "mouseup") ? "touchend" : (vt == "mousemove") ? "touchmove" : (vt == "touchstart") ? "mousedown" : (vt == "touchend") ? "mouseup" : (vt == "touchmove") ? "mousemove" : vt : vt;
	...
	var navigator = win.navigator,
		ua = function() {
			...
			var osA = os ? !jlpm.fn.isArray(os) ? os : os[1] == "cros" ? "chromeos" : os[1] : undefined || "",
				ovA = os ? !jlpm.fn.isArray(os) ? "0" : os[2] : undefined || "0",
				bsA = (browser ? jlpm.fn.isString(browser) ? browser : jlpm.fn.isArray(browser) && browser.length > 3 ? /safari/.test(browser[2]) ? browser[2] + browser[1] : browser[1] + browser[3] : browser[1] == "firefox" && /mobile/.test(ua) ? "firefoxmobile" : browser[1] : undefined) || "",
				bvA = (browser ? jlpm.fn.isString(browser) ? "0" : jlpm.fn.isArray(browser) && browser.length > 3 ? /safari/.test(browser[2]) ? browser[3] : browser[2] : browser[2] : undefined) || "0",
	...
	jlpm.fn.extend({
		...
		os : {},
		device : {}
	}).each(["msie", "webkit", "safari", "opera", "firefox", "firefoxmobile", "maxthon", "qqbrowser", "chrome", "baidubrowser", "wosbrowser", "chromemobile", "safarimobile"], function(i, name) {
		var self = jlpm.fn;
		self.extend(function(v) {
			var isb = ua().browser == name ? true : false;
			return (isb) ? (v == true) ? ua().browserver : true : false;
		}, {
			browser: name
		...
		var self = jlpm.fn;
		self.extend(function(v) {
			var isb = ua.os == name ? true : false;
			return (isb) ? v == true ?ua().osver : true : false;
		}, {
			os: name
		});
		...
		var self = jlpm.fn;
		self.extend(function(v) {
			var isb = ua().type == name ? true : false;
			return (isb) ? v == true ? ua().mode : true : false;
		}, {
			device: name
		});
	});
	...
	jlpm.fn.extend({
		readXmlNode: function(item, node, parent) {
			var m = item ? item : "item",
				...
				xmlnodes = [],
				self = this;
			this.domFind(m + (!this.isEmpty(sm) ? " " + sm : ""), rp).each(function() {
				var xmlnodesv = {},
				children = self.domFind(this).children(),
					tag;
				if (self.isEmpty(children[0])) {
					...
					self.domFind(this).children().each(function(i, obj) {
						tag = obj;
				...
				if (m.length) {
					this.each(m, function() {
						if (this.nodeType === 1) {
							if (!this.className && classNames.length === 1) {
								...
								this.className = self.trim(setClass);
							}
						}
					});
				} else {
					...
					if (elem.nodeType === 1) {
						if (!elem.className && classNames.length === 1) {
							...
							elem.className = self.trim(setClass);
						}
					}
				}
			}
		...
		toggleClass: function(className, m) {
			...
			this.each(m, function() {
				...
				self.each(cs, function() {
					sm.className = (self.hasClass(this, sm)) ? self.trim(sm.className.replace(self.classRE(this), '$1'), "all") : self.trim(sm.className + ' ' + this);
				});
			});
			return this;
		},
		...
		show: function(lineBool, m) {
			...
			this.each(m, function() {
				if (self.isHidden(this)) {
					this.style.display = self.domFind(this).attr("data-OldStyle") ? self.domFind(this).attr("data-OldStyle") : (ua().browser != "msie" && (this.tagName.toLowerCase() == "tr" || this.tagName.toLowerCase() == "td")) ? "table-row" : lineBool ? "inline-block" : "block";
					self.domFind(this).attr("data-OldStyle", "none");
				}
			});
			return this;
		},
		hide: function(lineBool, m) {
			...
			this.each(m, function() {
				if (self.isVisible(this)) {
					this.style.display = self.domFind(this).attr("data-OldStyle") ? self.domFind(this).attr("data-OldStyle") : 'none';
					self.domFind(this).attr("data-OldStyle", (ua().browser != "msie" && (this.tagName.toLowerCase() == "tr" || this.tagName.toLowerCase() == "td")) ? "table-row" : lineBool ? "inline-block" : "block");
				}
			});
			return this;
		},
	...
	jlpm.fn.extend({
		...
		textValue: function(value, splits, m) { //获取设置文本内容
			...
			if (!this.isEmpty(elems)) {
				if (this.isEmpty(gv)) {
					...
					return this.isEmpty(ret) ? undefined : ret;
				} else {
					if (elems.length) {
						this.each(elems, function() {
							if (this.nodeType === 3 || this.nodeType === 4) {
		...
		jlpm.fn.extend(function(selector, m) {
			var m = m ? m : this[0],
				gm = this.dir(m, type);
		...
		//0唯一的 //1过滤
		jlpm.fn.extend(function(value, m) {
			var m = m ? m : this[0],
				elem;
	...
	jlpm.fn.extend({
		...
		jlpm.fn.each({
			...
			jlpm.fn.extend(function(value, margin, m) {
	...
	if (ua().type != "desktop") {
		jlpm.fn.extend({
			...
			jlpm.fn.extend(function(callback, m) {
				var m = m ? m : this[0];
		...
		}).domFind(win).on("orientationchange", function() {
	...
	*/
	...
	(function() {
		...
		// EXPOSE
		jlpm.find = jlpm.fn.find = Sizzle;
		jlpm.oldFilter = jlpm.fn.oldFilter = Sizzle.filter;
		jlpm.oldunique = jlpm.fn.oldunique = Sizzle.uniqueSort;
		jlpm.expr = jlpm.fn.expr = Sizzle.selectors;
		//jlpm.expr[":"] = aaa.expr.filters;
		jlpm.text = jlpm.fn.text = Sizzle.getText;
		jlpm.isXMLDoc = jlpm.fn.isXMLDoc = Sizzle.isXML;
		jlpm.contains = jlpm.fn.contains = Sizzle.contains;
