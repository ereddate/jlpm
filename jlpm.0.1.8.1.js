...
(function(win) {
  var doc = win.document,
	...
	Array.prototype.remove=function(dx){  //增加删除功能
		if(isNaN(dx)||dx>this.length){return false;}
		var len = this.length,newdx = [];
		for(var i=0;i<len;i++){
	...
	jlpm.extend(jlpm.aid, {
		...
		record: function(old, ev, content, length, selector) {
			var df = {
				...
				length: length != undefined ? length : ev ? ev.length ? ev.length : 0 : 0,
		...
		crid: function(v, t) {
			if (typeof v != "undefined") {
				if (jlpm.aid.Doms._count >= 50) {
					jlpm.aid.Doms = jlpm.dispose(this.aid.Doms);
	...
	jlpm.extend({
		...
		type: function(v) {
			return v == null ? String(v) : this.aid.class2type[_toString.call(v)] || "object";
		},
		each: function(obj, callback, args) {
		...
		parseJSON: function(data) { //生成JSON
			...
			if (this.aid.sMatch.rvalidchars.test(data.replace(this.aid.sMatch.rvalidescape, "@").replace(this.aid.sMatch.rvalidtokens, "]").replace(this.aid.sMatch.rvalidbraces, ""))) {
	...
	jlpm.extend(jlpm.aid, {
		...
		ajaxLocParts: jlpm.aid.sMatch.rurl.exec(ajaxLocation.toLowerCase()) || []
	});
	jlpm.extend({
		...
		_cacheSet: function(key, value, bool) {
			...
			if (key){
				if (jlpm.isLocalStorage) {
					var m = win.localStorage,i,len,name;
					...
					if (jlpm.aid.cacheInitDom()) {
						...
						var m = jlpm.aid.cacheDataDom,i,len,name;
						for (i = 0; i < (len = jlpm.isArray(key) ? key.length : 1); i++) {
							name = jlpm.isArray(key) ? "jlpm_" + key[i] : "jlpm_" + key;
				...
				if (jlpm.isLocalStorage) {
					...
					if (jlpm.isArray(key)) {
						...
						jlpm.each(key, function() {
							var value = jlpm.evalexec(win.localStorage.getItem("jlpm_" + this));
							value = jlpm.isString(value) ? value.split(',') : value;
							if (value && value[0]+"" == bool+"") rvalue.push(jlpm.evalexec(value.length>2 ? value.remove(0) : value[1]));
						});
					} else {
						var value = jlpm.evalexec(win.localStorage.getItem("jlpm_" + this));
						value = jlpm.isString(value) ? value.split(',') : value;
						if (value && value[0]+"" == bool+"") rvalue = jlpm.evalexec(value.length>2 ? value.remove(0) : value[1]);
					}
					...
					if (jlpm.aid.cacheInitDom()) {
						...
						if (jlpm.isArray(key)) {
							...
							jlpm.each(key, function() {
								var value = jlpm.evalexec(m.getAttribute("jlpm_" + this));
								value = jlpm.isString(value) ? value.split(',') : value;
								if (value && value[0]+"" == bool+"") rvalue.push(jlpm.evalexec(value.length>2 ? value.remove(0) : value[1]));
							});
						} else {
							var value = jlpm.evalexec(jlpm.aid.cacheDataDom.getAttribute("jlpm_" + key));
							value = jlpm.isString(value) ? value.split(',') : value;
							if (value && value[0]+"" == bool+"") rvalue = jlpm.evalexec(value.length>2 ? value.remove(0) : value[1]);
						}
						doc.body.removeChild(jlpm.aid.cacheDataDom);
	...
	jlpm.each(jlpm.aid.evenList, function(i, name) {
		jlpm.fn.extend({
			events: name
		}, function(fn, m) {
			var m = m ? m : jlpm[0],
	...
	jlpm.browser.flash = function(v) {
		...
		if (version != undefined) isFlash = true;
		return (v == true) ? version : isFlash;
	};
	...
	jlpm.extend({
		...
		camelCase: function(string) {
			return string.replace(this.aid.sMatch.rmsPrefix, "ms-").replace(this.aid.sMatch.rdashAlpha, this.fcamelCase);
		}
	});
		...
		_getAttr: function(name, value, m) {
			...
			if (jlpm.type(name) == "string") {
				...
				if (!jlpm.isEmpty(value) && jlpm.type(value) == "string") {
					...
					jlpm.each(names, function(i, name) {
						var vnvalue = (/data\-/.test(name)) ? self.aid._dataAttr(m, self.camelCase(name.substring(5))) : m[self.aid.propfx[name] || name];
					...
					return jlpm.isEmpty(values) ? undefined : values;
				}
			} else {
	...
	jlpm.extend(jlpm.aid, {
		...
		_cssPrefix: /webkit|chrome|chromemobile|safari|safarimobile/.test(jlpm.ua().browser) ? "Webkit" : /firefox|firefoxmobile/.test(jlpm.ua().browser) ? "Moz" : /msie/.test(jlpm.ua().browser) ? "ms" : /opera/.test(jlpm.ua().browser) ? "O" : "",
	...
	jlpm.extend(jlpm.aid, {
		dir: function(elem, dir, until) {
			...
			if (jlpm.isNodeList(cur) || jlpm.isObjectList(cur)) {
				try {
					matched = jlpm.toArray(cur);
				} catch (e) {
					matched = gcur(cur);
				}
				jlpm.each(matched, function(i) {
					var push = self.aid.dir(this, dir);
	...
	jlpm.fn.extend({
		...
		childNodes: function(bool, m) {
			var m = m ? m : jlpm[0],
				...
				self = this,
				vgm = function(m) {
					var v = jlpm.aid.dir(m, "childNodes"),
						matchjson = {},
						i, name;
				...
				gm = bool == false ? vgm.call(this, m) : jlpm.aid.dir(m, "childNodes");
			if (gm) {
	...
	jlpm.each(["siblings", "children"], function(i, name) {
		...
		jlpm.fn.extend(name, function(m) {
			...
			var elems = [];
			jlpm.each(m, function() {
				elems = this.parentNode && /0/.test(i + "") ? elems.concat(jlpm.aid._siblings(this.parentNode.firstChild, this)) : elems.concat(jlpm.aid._siblings(this.firstChild));
			});
			elems = jlpm.isSingle(elems) ? elems[0] : elems;
			jlpm.aid.record(m, elems, null, null);
			return this;
		...
		jlpm.fn.extend(name, function(m) {
			var m = m ? m : jlpm[0],
				...
				self = this;
			if (m && m.nodeType) {
				a.push(jlpm.aid.eq(/0/.test(i + "") ? 0 : (self.children(m)[0] ? self.children(m)[0].length - 1 : 0), self.children(m)[0]));
			} else {
				jlpm.each(m, function(obj) {
					a.push(jlpm.aid.eq(/0/.test(i + "") ? 0 : self.length - 1, obj));
				});
			}
			if (jlpm.isSingle(a)) a = a[0];
			if (jlpm.isEmpty(a)) return this;
	...
	jlpm.extend(jlpm.aid, {
		...
		_getWH: function(elem, name, computed, extra) {
			...
			if (computed) {
				...
				return elem["offset" + jlpm.aid.propfx[name]] === 0 && jlpm.aid.sMatch.rdisplayswap.test(jlpm(elem).css("display")) ? jlpm(elem).swap(jlpm.aid.propfx["cssShow"], function() {
	...
	if (jlpm.ua().browser == "msie" && jlpm.ua().type == "desktop") {
