...
(function(win) {
				...
				jlpm.each(va, function(i, name) {
					...
					jlpm.each(m, function(i, obj) {
						...
						if (jlpm.eventHandlers[id + "_" + vname]) {
							var fn = jlpm.eventHandlers[id + "_" + vname].m;
	...
	jlpm.extend({
		...
		isTouch: function() {
			...
			if (('ontouchstart' in win) || win.DocumentTouch && doc instanceof DocumentTouch) {
				...
				jlpm._injectElementWithStyles(['@media (', prefixes.join('touch-enabled),('), mod, ')', '{#modernizr{top:9px;position:absolute}}'].join(''), function(node) {
	...
	jlpm.extend({
		...
		_cssFix: function(obj, name, value) {
			if (value) {
				obj.style[jlpm._cssPrefix + name.capitalize()] != undefined ? obj.style[jlpm._cssPrefix + name.capitalize()] = value : obj.style[name] = value;
			} else {
				return obj.style[jlpm._cssPrefix + name.capitalize()] != undefined ? obj.style[jlpm._cssPrefix + name.capitalize()] : obj.style[name];
			}
		},
				...
				if (!this.isEmpty(value) && (this.isString(value) || this.isNumeric(value))) {
					...
					this.each(names, function(i, name) {
						if (self.isSingle(names)) {
							if (fo.test(name.toLowerCase())) {
								...
								jlpm._cssFix(m, name, value);
							}
							return false;
						} else {
							jlpm._cssFix(m, name, values[i]);
						}
					});
					return true;
				} else if (this.isEmpty(value)) {
					this.each(names, function(i, name) {
						...
						if (fo.test(name.toLowerCase())) {
							...
							values.push((self.getStyles(m) ? self.getStyles(m)[name] : undefined) || (m.style ? jlpm._cssFix(m, name) : undefined) || m["offset" + jlpm.propfx[name]]);
						}
					});
	...
	jlpm(win).off("unload").on("unload", function() {
		jlpm.each("Doms class2type configs _callbacks ajaxhandlers eventHandlers mhandlers classCache".split(' '), function(i, name) {
			jlpm.dispose(jlpm[name] || jlpm.fn[name]);
		});
