	...
	jlpm.extend({
		...
		config: function() { //配置外部插件
			...
			jlpm.each(obj, function(name, value) {
				if (value) {
					target.extend(name, self.isFunction(obj[name]) ? obj[name] : function() {
						return jlpm.loadon(value && value["url"] ? value["url"] : "", function() {
			...
			ajax.fn = ajax.prototype = {
				...
				send: function(url, method, callback) {
					...
					var dataType = this.dataType,
						...
						result = self.oninit(data ? self.stringtojson(data) : data);
